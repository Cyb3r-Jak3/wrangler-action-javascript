...
export async function wrangler_run(): Promise<void> {
  ...
  const publish_output = await exec.exec('wrangler', ['publish', environment])
  if (publish_output !== 0) {
    throw new Error('Publish command did not complete successfully')
  }
  secrets.forEach(async element => {
    const secret_output = await exec.getExecOutput('wrangler', [
      'secret',
      'put',
      element,
      core.getInput(element),
      environment
    ])
    if (secret_output.exitCode !== 0 && fail_on_missing_secret) {
